    ...
    <div class="panel h-full my-3">
        <div class="mb-5 flex items-start border-b border-[#e0e6ed] py-3 dark:border-[#1b2e4b]">
            <div class="shrink-0 rounded-full ring-2 ring-white-light ltr:mr-4 rtl:ml-4 dark:ring-dark">
                <img :src="post.user.photoURL ?? '/app/images/avatar-placeholder.jpg'" alt="" class="h-10 w-10 rounded-full object-cover" />
            </div>
            <div class="font-semibold">
                <NuxtLink :to="post.user.id === store.user.id ? '/app/profile' : `/app/profile/${post.user.id}`">
        ...
        <div>
            ...
            <div class="bottom-0 flex w-full items-center justify-between py-5">
                <div class="flex items-center">
                    <icon-heart @click="$emit('like')" class="w-5 h-5 mr-1 text-danger hover:fill-danger inline relative cursor-pointer" :class="{'fill-danger':post.likes.includes(store.user.id)}" />
